...
/* Some embedded / private browsers throw on storage access — never let that crash the shop. */
const store = {
...
const readAccounts = (): DemoAccount[] => {
  try {
    return JSON.parse(store.get("foa_accounts") ?? "[]") as DemoAccount[];
  } catch {
    return [];
  }
};
const writeAccounts = (a: DemoAccount[]) => store.set("foa_accounts", JSON.stringify(a));
const readSession = (): string | null => store.get("foa_session");
