...
import { categoryMeta, categoryTiles, products, tileCount } from "../data";
...
export default function CategoryPage({ cat }: { cat: string }) {
  ...
  const tile = categoryTiles.find((t) => t.name === cat);
  const fallbackMeta = categoryMeta.find((c) => c.name === cat);
  const meta = tile
    ? { name: tile.name, img: tile.img, blurb: tile.blurb, items: `${tileCount(tile)} picks` }
    : fallbackMeta
      ? { ...fallbackMeta }
      : null;
