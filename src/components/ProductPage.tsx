...
export default function ProductPage({ id }: { id: string }) {
  ...
  const specs = useMemo(() => (p ? (p.materials?.length ? p.materials : specsFor(p)) : []), [p]);
  const careRows = useMemo(() => (p?.care?.length ? p.care.map((t) => ({ title: t, text: "" })) : CARE), [p]);
  const maker = useMemo(() => (p ? makerFor(p) : null), [p]);

  if (!p || !maker) return null;
  const wished = isWished(p.id);
  // portal-set links win; then the built-in checkout map
  const externalCheckout = (p.buyLink && p.buyLink.trim()) || EXTERNAL_CHECKOUT[p.id];
        ...
        >
          ...
          <div className="mt-4 border-t border-dashed border-slate-200 pt-3">
            ...
            <div className="mt-2 space-y-2">
              {careRows.map((c, i) => {
                const Icon = CARE_ICONS[i % CARE_ICONS.length];
                return (
        ...
        <AnimatePresence mode="wait">
          {tab === "desc" ? (
            <motion.div key="desc" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.28 }} className="pt-4">
              ...
              {p.details && (
                <p className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-[10.5px] font-semibold text-slate-600 ring-1 ring-slate-200/70">{p.details}</p>
              )}
